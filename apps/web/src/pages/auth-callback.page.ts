 ...
 *
 * Le hash (#access_token=...) est lu ici AVANT que le router ne navigue.
 *
 * detectSessionInUrl ne se déclenche QUE lors d'un load complet du SDK.
 * En navigation SPA (push/popState), les tokens dans window.location.hash
 * ou .search ne sont jamais extraits → la page reste en attente jusqu'au
 * timeout. On parse manuellement et on appelle setSession() ou
 * exchangeCodeForSession() pour fiabiliser.
 *
 * Le timer de 10s est stocké et annulé (destroy()) pour éviter les
 * mutations de state sur un composant démonté.
 *
 * APK about:blank : Sur Capacitor, callbackUrl() pointe vers /auth-callback
 * HTTPS (et non plus le custom scheme com.lamajoker.kinetic://). Chrome
 * Custom Tabs ne peut pas naviguer vers un custom scheme après OAuth →
 * about:blank. Le flow corrigé :
 *   1. Google/Supabase redirige vers https://…/auth-callback (OK dans CTab)
 ...
 *      pour que l'APK reprenne la main (Intent filter Android)
...
export function authCallback() {
  return {
    ...
    async init(): Promise<void> {
      try {
        ...
        if (accessToken && refreshToken) {
          // Si on est dans un WebView Capacitor, cette page a été ouverte dans
          // Chrome Custom Tabs. On hand-off les tokens vers l'APK via le deep
          // link → l'Intent filter Android ouvre l'APK.
          if (_isCapacitor) {
            const deepLink =
    ...
    /** Annuler timer + subscription pour éviter les mutations sur composant démonté */
    _cleanup(): void {

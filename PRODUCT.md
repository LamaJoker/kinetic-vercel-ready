...
- [ ] Brancher `suggestProgression()` dans `seances.page.ts` sous chaque exo
- [ ] Écran "Progression" avec graphes (Chart.js ou canvas natif)
- [ ] Bouton "Exporter mes données" dans Profil

### Phase 2 — 1 mois

- [ ] Génération automatique de templates depuis un split choisi
- [ ] Push Web réel (Service Worker + VAPID, rappel streak H-4)
- [ ] Objectifs hebdomadaires (3 séances, X kg tonnage) avec XP bonus
- [ ] Migration SQL `004_training_tables.sql` + sync HybridStorage pour workouts

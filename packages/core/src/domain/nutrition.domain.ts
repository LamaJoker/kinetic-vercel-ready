...
export function buildNutritionPlan(m: UserMeasurements): NutritionPlan {
  ...
  const deltas: Record<Goal, number> = {
    lose_fat: Math.max(-700, Math.round(-tdee * 0.2)),
  ...
  if (m.goal === 'lose_fat')
    notes.push('Cible : −0.5 à 1 % du poids / semaine (préserver la masse).');
  if (m.goal === 'gain_muscle')
    notes.push('Cible : +0.25–0.5 kg / semaine novice, +0.1 kg intermédiaire.');
  ...
  return {
    ...
    macros: {
      ...
      waterMl: Math.round(
        m.weightKg * 35 + (['active', 'very_active'].includes(m.activity) ? 500 : 0),
      ),
    },
    notes,
...
export function mealTimingPlan(plan: NutritionPlan, training: boolean): MealTiming[] {
  ...
  if (!training)
    return [
      {
        mealName: 'Matin',
        kcal: Math.round(m.kcal * 0.35),
        proteinG: Math.round(m.proteinG * 0.34),
        carbsG: Math.round(m.carbsG * 0.4),
        fatG: Math.round(m.fatG * 0.3),
        notes: "Œufs, fromage blanc, flocons d'avoine.",
      },
      {
        mealName: 'Déjeuner',
        kcal: Math.round(m.kcal * 0.35),
        proteinG: Math.round(m.proteinG * 0.34),
        carbsG: Math.round(m.carbsG * 0.35),
        fatG: Math.round(m.fatG * 0.35),
        notes: 'Repas équilibré — glucides légèrement réduits jour repos.',
      },
      {
        mealName: 'Dîner',
        kcal: Math.round(m.kcal * 0.3),
        proteinG: Math.round(m.proteinG * 0.32),
        carbsG: Math.round(m.carbsG * 0.25),
        fatG: Math.round(m.fatG * 0.35),
        notes: 'Protéines lentes (caséine, œufs), légumes à volonté.',
      },
    ];
  return [
    {
      mealName: 'Matin',
      kcal: Math.round(m.kcal * 0.28),
      proteinG: Math.round(m.proteinG * 0.24),
      carbsG: Math.round(m.carbsG * 0.28),
      fatG: Math.round(m.fatG * 0.28),
      notes: 'Petit-déjeuner complet.',
    },
    {
      mealName: 'Pré-séance (−90 min)',
      kcal: Math.round(m.kcal * 0.2),
      proteinG: Math.round(m.proteinG * 0.2),
      carbsG: Math.round(m.carbsG * 0.3),
      fatG: Math.round(m.fatG * 0.1),
      notes: 'Glucides complexes + protéines légères. Limiter lipides.',
    },
    {
      mealName: 'Post-séance (< 2 h)',
      kcal: Math.round(m.kcal * 0.25),
      proteinG: Math.round(m.proteinG * 0.3),
      carbsG: Math.round(m.carbsG * 0.25),
      fatG: Math.round(m.fatG * 0.15),
      notes: 'Fenêtre anabolique : whey + glucides rapides.',
    },
    {
      mealName: 'Dîner',
      kcal: Math.round(m.kcal * 0.27),
      proteinG: Math.round(m.proteinG * 0.26),
      carbsG: Math.round(m.carbsG * 0.17),
      fatG: Math.round(m.fatG * 0.47),
      notes: 'Caséine ou œufs — lipides OK le soir.',
    },
  ];
}
